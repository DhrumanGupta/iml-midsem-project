...
class TransformerModel(nn.Module):
    def __init__(self, input_size, is_deltas, hidden_size=128, num_layers=8, num_heads=8, dropout=0.2):
        """
        Parameters:
    ...
    def forward(self, x_sir, x_interventions, x_static, return_sequence=False):
        ...
        if self.is_deltas:
            ...
            if x_out.dim() == 3:
                ...
                students_probs = students_logits / (students_logits.sum(dim=2, keepdim=True) + 1e-8)
                adults_probs = adults_logits / (adults_logits.sum(dim=2, keepdim=True) + 1e-8)
                processed = torch.cat((students_probs, adults_probs), dim=2)
            else:
                students_logits = torch.sigmoid(x_out[:, :3])
                adults_logits = torch.sigmoid(x_out[:, 3:])
                students_probs = students_logits / (students_logits.sum(dim=1, keepdim=True) + 1e-8)
                adults_probs = adults_logits / (adults_logits.sum(dim=1, keepdim=True) + 1e-8)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_pbar:
            ...
            if x_sir.dim() == 3:
                ...
                preds_seq = model(x_sir, x_interventions, x_static, return_sequence=True)
